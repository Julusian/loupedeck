...
describe('Connection Management', () => {
    ...
    it('attempts reconnect if device not found', async() => {
        const serialDiscovery = jest.spyOn(SerialConnection, 'discover').mockImplementation(() => { return [] })
        const wsDiscovery = jest.spyOn(WSConnection, 'discover').mockImplementation(() => { return [] })
        const fn = jest.fn()
        device = new LoupedeckDevice({ autoConnect: false })
